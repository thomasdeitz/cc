const Dates = {
    ...
    computed: {
        sortedShows: function () {
            ...
            console.log('2026 ', showCountShows.filter((show) => {return new Date(show.datetime) >= new Date("2026-01-01T00:00") && new Date(show.datetime) < new Date("2027-01-01T00:00")}).length);
            console.log('2026 so far', showCountShows.filter((show) => {return new Date(show.datetime) >= new Date("2026-01-01T00:00") && new Date(show.datetime) < this.today}).length);
